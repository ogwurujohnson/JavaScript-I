...
for (let i = 0; i < inventory.length; i++) {
    if(inventory[i].id === inventory.length) {
        console.log(
        `Car ${inventory.length} is a ${inventory[i].car_year} ${inventory[i].car_make} ${inventory[i].car_model}`
        );
    }
}
...
let carModels = [];
for (let i = 0; i < inventory.length; i++) {
    carModels.push(Object.values(inventory[i])[2]);
}
...
let carYears = [];
for(let i = 0; i < inventory.length; i++) {
    carYears.push(Object.values(inventory[i])[3])
}
...
let oldCars = [];
for(let i = 0; i < inventory.length; i++) {
    if(Object.values(inventory[i])[3] > 2000) {
...
let BMWAndAudi = [];
for(let i = 0; i < inventory.length; i++) {
    if(Object.values(inventory[i])[1] === 'BMW' || Object.values(inventory[i])[1] === 'Audi') {
